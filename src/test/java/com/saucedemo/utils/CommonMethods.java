...
public static void benchmarkingLazyLoading() throws InterruptedException {
	...
	public static WebElement waitForVisibleFluentWait(WebElement element) {
		final Wait<WebDriver> wait = getDefaultFluentWait().ignoring(NoSuchElementException.class)
	...
	private static FluentWait<WebDriver> getDefaultFluentWait() {
		return getCustomFluentWait(30, 30);
	}

	private static FluentWait<WebDriver> getCustomFluentWait(int waitTime, long seconds) {
		return new FluentWait<>(driver).withTimeout(waitTime, TimeUnit.SECONDS).pollingEvery(seconds, TimeUnit.SECONDS);
	}
